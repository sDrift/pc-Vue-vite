  ...
  <div class="async-loader-test">
    <el-card shadow="hover" class="main-card">
      ...
      <div class="render-area">
        <!-- v-for：累加显示所有已激活的组件 -->
        <div v-for="c in currents" :key="c.key" class="render-card">
          <div class="render-header">
            <span class="render-title">{{ c.key.toUpperCase() }} · {{ labels[c.key] }}</span>
            <el-button text size="small" type="danger" @click="unmount(c.key)">卸载</el-button>
          </div>
          <component :is="c.comp" />
        </div>
        ...
        <div v-if="!currents.length" class="placeholder">
          ...
          <p class="small">
            💡 点击多个按钮后，所有已加载的组件会同时显示在下方，互不影响
            <br />
            打开浏览器 DevTools → Network → 切到 JS 分类，
            <br />
            会看到
            <code>ChildA.[hash].js</code>
            /
            <code>ChildB.[hash].js</code>
            /
            <code>ChildC.[hash].js</code>
            只在点击对应按钮后才会出现
          </p>
        </div>
      </div>
...
const reloadAsync = (key) => {
  ...
  currents.value = currents.value.map((c) => (c.key === key ? { key, comp: cache[key] } : c))
}
