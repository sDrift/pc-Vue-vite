    ...
    <div class="number-input-wrapper">
      ...
      <div class="select-pair-container">
        <div class="select-controls">
          <el-button type="primary" class="control-btn" @click="openBothSelects">
            同时打开两个选择器
          </el-button>
          <el-button class="control-btn" @click="focusSelect1">聚焦到第一个选择器</el-button>
          <el-button class="control-btn" @click="focusSelect2">聚焦到第二个选择器</el-button>
          <el-button class="control-btn" @click="closeBothSelects">关闭两个选择器</el-button>
        </div>
      ...
      <div v-if="inputNumber !== null" class="input-value">输入的值: {{ inputNumber }}</div>
      ...
      <div class="radio-group-wrapper">
        ...
        <div v-if="radioValue" class="radio-value">选择的值: {{ radioValue }}</div>
        ...
        <div id="print-test-div" ref="printDivRef" class="print-test-div">
          ...
          <div class="print-content">
            ...
            <div v-if="radioValue" class="print-radio-info">您选择的选项是: {{ radioValue }}</div>
          </div>
        </div>
      </div>
    </div>
...
const showSelectDropdown = async (selectRef, isFirst = false) => {
  try {
    ...
    const isFirstSelect = selectRef === select1Ref.value
    ...
    if (!popperElement) {
      ...
      for (const popper of allPoppers) {
        // 检查是否与当前选择器关联
        const relatedSelect =
          popper.getAttribute('x-placement') || popper.getAttribute('data-select')
...
const keepDropdownsVisible = () => {
  ...
  const popperElements = document.querySelectorAll(
    '.el-popper.select1-popper, .el-popper.select2-popper',
  )
...
const preventSelectClose = (event) => {
  // 检查是否点击了选择器或其下拉框内部
  const isSelectArea =
    event.target.closest('.el-select.keep-dropdown-open') ||
