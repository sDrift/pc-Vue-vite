        ...
        <template #header>
          <div class="card-header">
            ...
            <el-space>
              ...
              <el-button size="small" type="warning" @click="loadPreset('user')">用户表单</el-button>
              <el-button size="small" type="warning" @click="loadPreset('order')">订单表单</el-button>
              <el-button size="small" type="warning" @click="loadPreset('survey')">问卷表单</el-button>
            </el-space>
          </div>
        </template>
          ...
          >
            ...
            <el-row :gutter="10" class="field-row-body link-row">
              ...
              <template v-if="item.visibleWhen">
                ...
                <el-col :span="8" v-if="!['truthy','falsy'].includes(item.visibleWhen.op)">
                  <el-input
            ...
            >
              <el-col :span="8">
                <el-input-number v-model="item.min" placeholder="最小值" size="small" style="width:100%" />
              </el-col>
              <el-col :span="8">
                <el-input-number v-model="item.max" placeholder="最大值" size="small" style="width:100%" />
              </el-col>
              <el-col :span="8">
                <el-input-number v-model="item.step" placeholder="步长" size="small" style="width:100%" />
              </el-col>
            </el-row>
          ...
          <template #header>
            <div class="card-header">
              ...
              <el-space>
                ...
                <el-input-number v-model="gutter" :min="0" :max="40" size="small" controls-position="small" />
...
const commitVisibleWhenValue = (item) => {
  ...
  if (['in', 'notIn'].includes(op)) {
    val = text.split(',').map((s) => s.trim()).filter(Boolean);
...
const presets = {
  user: [
    { field: 'username', label: '用户名', type: 'input', required: true, span: 12, maxlength: 20, tip: '4-20 个字符' },
    ...
    { field: 'gender', label: '性别', type: 'radio', span: 12, options: [
      { label: '男', value: 'male' },
      { label: '女', value: 'female' },
    ] },
    { field: 'hobby', label: '爱好', type: 'checkbox', span: 12, options: [
      { label: '阅读', value: 'read' },
      { label: '运动', value: 'sport' },
      { label: '音乐', value: 'music' },
    ] },
    { field: 'intro', label: '个人简介', type: 'textarea', span: 24, maxlength: 200, tip: '最多 200 字' },
  ],
  order: [
    { field: 'orderNo', label: '订单号', type: 'input', required: true, span: 12, default: 'ORD-001' },
    { field: 'orderType', label: '订单类型', type: 'select', span: 12, options: [
  ...
  survey: [
    { field: 'satisfaction', label: '满意度', type: 'rate', span: 24, max: 10, showText: true, texts: ['极差','失望','一般','满意','惊喜'], default: 5 },

    /* —— 联动 1：是否推荐 → 显示"推荐理由" —— */
    { field: 'recommend', label: '是否推荐', type: 'switch', span: 12, activeText: '会', inactiveText: '不会' },
    { field: 'recommend_reason', label: '推荐理由', type: 'textarea', span: 24, maxlength: 200,
      tip: '只有"是否推荐"打开时才显示',
      visibleWhen: { field: 'recommend', op: 'eq', value: true },
    },

    /* —— 联动 2：使用频率 → 显示"使用场景" —— */
    { field: 'frequency', label: '使用频率', type: 'radio', span: 12, options: [
      { label: '每天', value: 'daily' },
      { label: '每周', value: 'weekly' },
      { label: '每月', value: 'monthly' },
      { label: '偶尔', value: 'rarely' },
    ] },
    { field: 'frequency_other', label: '其他频率', type: 'input', span: 12, placeholder: '请填写',
      visibleWhen: { field: 'frequency', op: 'eq', value: 'rarely' },
    },

    /* —— 联动 3：了解渠道（多选）包含"朋友推荐" → 显示"推荐人姓名" —— */
    { field: 'channel', label: '了解渠道', type: 'checkbox', span: 24, options: [
      { label: '搜索引擎', value: 'search' },
      { label: '社交媒体', value: 'social' },
      { label: '朋友推荐', value: 'friend' },
      { label: '广告', value: 'ad' },
    ] },
    { field: 'referrer', label: '推荐人姓名', type: 'input', span: 12, required: true,
      tip: '勾选了"朋友推荐"才会出现，且为必填',
      visibleWhen: { field: 'channel', op: 'contains', value: 'friend' },
    },

    /* —— 联动 4：评分 ≥ 8 才显示"高赞评价" —— */
    { field: 'praise', label: '高赞评价', type: 'textarea', span: 24, maxlength: 300,
      tip: '满意度 ≥ 8 分才显示',
      visibleWhen: { field: 'satisfaction', op: 'gte', value: 8 },
    },

    { field: 'theme', label: '主题色', type: 'color', span: 12, default: '#409eff' },
    { field: 'feedback', label: '反馈意见', type: 'textarea', span: 24, maxlength: 500, required: true },
  ],
};
