  ...
  <div class="underline-animation-container">
    <div class="text-container">
      <div ref="textWrapper" class="text-wrapper">
        <!-- 文本行 -->
        <div v-for="(line, index) in lines" :key="index" ref="lineRefs" class="text-line">
          {{ line }}
          <!-- 下划线 -->
          <div
    ...
    <button :disabled="isAnimating" class="start-button" @click="startAnimation">
...
// 文本内容
const text =
  '这是一段示例文字，点击下面的按钮后，这段文字将在5秒内从开头到结尾逐渐显示下划线动画效果。这段文字将会自动换行显示为多行，下划线动画将从第一行开始，一行一行地显示，直到最后一行完成。每行都会有自己的下划线，并且会按照顺序依次出现，形成一个流畅的逐行下划线动画效果。'
...
.line-underline {
  ...
  height: 4px;
  width: 0%;
  background: url("data:image/svg+xml,%3Csvg width='100' height='4' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M 0,2 Q 5,4 10,2 T 20,2 T 30,2 T 40,2 T 50,2 T 60,2 T 70,2 T 80,2 T 90,2 T 100,2' fill='none' stroke='%23ff0000' stroke-width='2'/%3E%3C/svg%3E")
    repeat-x 0 bottom/20px 100%;
  transition:
    width 0.05s linear,
