...
function getPie3D(pieData, internalDiameterRatio) {
  ...
  const k = typeof internalDiameterRatio !== 'undefined' ? 
    (1 - safeInternalDiameterRatio) / (1 + safeInternalDiameterRatio) : 1 / 3;
...
const initChart = () => {
  ...
  // 使用props传入的数据或默认数据
  const pieData = props.data.length > 0 ? props.data : [
    {
      name: 'cc',
      value: 47,
      itemStyle: {
...
const updateChart = (newData = null) => {
  ...
  const pieData = newData && newData.length > 0 ? newData : props.data.length > 0 ? props.data : option.series.map(item => item.pieData);
