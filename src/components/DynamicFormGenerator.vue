<template>
  ...
  >
    <el-row :gutter="gutter">
      ...
      <!-- 提交按钮 -->
      <el-col :span="24" v-if="showSubmit">
        <el-form-item>
...
const SUPPORTED_TYPES = [
  ...
  'switch', 'slider', 'rate', 'date', 'time', 'color', 'cascader',
];
...
const checkCondition = (cond, data) => {
  ...
  switch (cond.op) {
    ...
    case 'truthy': return !!actual && !(Array.isArray(actual) && actual.length === 0);
    case 'falsy': return !actual || (Array.isArray(actual) && actual.length === 0);
    default: return true;
  }
};
  ...
  (state) => {
    props.schema.forEach((item) => {
      if (state[item.field] === false && item.clearWhenHidden !== false) {
        ...
        else if (['number', 'rate', 'slider'].includes(item.type)) formData[item.field] = item.min ?? 0;
