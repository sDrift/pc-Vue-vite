  ...
  <div class="dashboard-container">
    ...
    <div v-if="selectedCity" class="city-detail">
      <div class="detail-header">
        ...
        <button @click="resetSelectedCity" class="close-btn">×</button>
      </div>
...
const initMap = async () => {
  ...
  try {
    ...
    // 使用默认SVG路径作为后备
    planeBase64Image.value = 'path://M1705.06,1318.313v-89.254l-319.9-221.799l0.073-208.063c0.521-84.662-26.629-121.796-63.961-121.491c-37.332-0.305-64.482,36.829-63.961,121.491l0.073,208.063l-319.9,221.799v89.254l319.9,221.799l0.073,208.063c0.521,84.662,26.629,121.796,63.961,121.491c37.332-0.305,64.482-36.829,63.961-121.491l0.073-208.063L1705.06,1318.313z';
  }
  ...
  const option = {
    ...
    geo: {
      ...
      tooltip: {
        formatter: function(params) {
          let data = cityData.value.find(city => city.name.includes(params.name))||{};
  ...
  const addAnimationPath = (pathData) => {
    if (pathData && pathData.segments && Array.isArray(pathData.segments) && pathData.segments.length > 0) {
      // 使用push方式添加新路径
